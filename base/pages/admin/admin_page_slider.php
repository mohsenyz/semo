...
if(isset($_POST['delete'])){
    $query = $conn->query("DELETE FROM `slider` WHERE `id`=" . $_POST['id']);
    unlink('ex/photo/slider/' . $_POST['img']);
    $text = '<form action="' . $addres . 'admin/slider' . '" method="post"><h1 style="color: green">Successful</h1><input type="submit" value="Back" name="edit" /></form>';
}
if(isset($_POST['state'])){
    if($_POST['state'] == 'Inactive'){
        $query = $conn->query("UPDATE `slider` SET `ac`=0 WHERE `id`=" . $_POST['id']);
    }  else {
        $query = $conn->query("UPDATE `slider` SET `ac`=1 WHERE `id`=" . $_POST['id']);
    }
    $text = '<form action="' . $addres . 'admin/slider' . '" method="post"><h1 style="color: green">Successful</h1><input type="submit" value="Back" name="edit" /></form>';
}
?>
    ...
    <body>
        ...
        <table style="width: 100%;direction: rtl;">
            ...
            <tr style="text-align: center;">
                ...
                <td><a href="../ex/photo/slider/<?php echo $fetch['img']; ?>" target="_blank"><img src="../ex/photo/slider/<?php echo $fetch['img']; ?>" width="75px" /></a></td>
                <td><?php if($fetch['ac'] == 1){echo '<h3 style="color: green">Active</h3>';}  else {echo '<h3 style="color: red">Inactive</h3>';} ?></td>
                <td>
                    <form action="<?php echo $addres . 'admin/opsr'; ?>" method="post"><input type="submit" name="edit" value="Edit" /><input type="hidden" name="id" value="<?php echo $fetch['id']; ?>" /></form>
        ...
        <a href="<?php echo $addres . 'admin/adsr'; ?>">Add a new Image</a><br /><a href="<?php echo $addres . 'admin/'; ?>">Back</a>
        <?php } ?>
    </body>
</html>